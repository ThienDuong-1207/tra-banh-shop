...
| `OrdersView` | `"use client"` — view "Đơn hàng" (nav `donhang` trong `HomeClient.tsx`). Đọc/cập nhật trực tiếp bảng `orders`/`order_items` (RLS đã cấp sẵn cho nhân sự nội bộ từ `supabase/migrations/001_orders_and_public_products.sql`), subscribe realtime `postgres_changes` trên `orders` (INSERT/UPDATE) cùng pattern với `NotificationBell`. Dải KPI đầu trang (đơn hôm nay, doanh thu 7 ngày, chờ thanh toán) qua `.kpi-grid`/`.kpi-card`. Tab lọc theo `OrderStatus` qua `Segmented`, panel trượt phải (`.panel-backdrop`/`.panel-slide` — KHÔNG dùng `.modal`, xem lý do trong `app/(admin)/globals.css`) xem `order_items` + đổi trạng thái (ghi `confirmed_at`/`confirmed_by` đúng 1 lần ở lần đổi đầu tiên) |
| `OverviewView` | `"use client"` — view "Tổng quan" (nav `tongquan`, **landing page mặc định** khi đăng nhập, thay `hanghoa` trước đây). Tự fetch `orders` riêng (không lift lên `HomeClient`, theo đúng pattern các view khác). KPI đơn/doanh thu tháng này + sản phẩm thiếu giá (nhận `products` qua prop, không fetch lại), bar-chart đơn theo `OrderStatus` (dùng lại `.panels`/`.bar-row` đã có ở "Báo cáo"), danh sách 6 đơn gần nhất + nút `onNavigate("donhang")` sang `OrdersView`. Khác "Báo cáo" (`DashboardView`, view `baocao` — vẫn giữ nguyên, chỉ xoay quanh sản phẩm/lịch sử giá) — không trùng nội dung |
